...
//add to record target test cases
    if(getSharedMemory() == 32){
      ...
      sprintf(fileName, "/testcases/file_i_%ld.txt", elapsed);
      ...
      fprintf(f, "the first test case is : %s\n", bytes.data());
      fclose(f);
      setSharedMemory(0);
    }
//end
...
//add to record target test cases
      if(getSharedMemory() == 32){
        char fileName[32];
        long elapsed = unit_start_time.tv_sec*1000000 + unit_start_time.tv_usec;
        sprintf(fileName, "/testcases/file%ld.txt", elapsed);
        FILE *f = fopen(fileName, "w");
        if (f == NULL)
        {
          printf("Error opening file!\n");
